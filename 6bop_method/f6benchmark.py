...
from e3model_exp import GMGPVNet

# ================= 配置 =================
CONFIG = {
    "model_path": "./cloudcheckpoint1223/all_objects_v1/last.pth", 
    ...
    "vis_dir": "./benchmark_vis/final_allobj" # 可视化保存路径
}
class Visualizer:
...
class BenchmarkRunner:
    ...
    def get_real_box_pts(self, obj_id):
            # 粗略估计的半径 (mm)
        sizes = {
            1: 80, 2: 100, 3: 90, 4: 60, 5: 80, 
            9: 50, 10: 80, 13: 80, 21: 60
        }
        s = sizes.get(int(obj_id), 70.0) # 默认 70
        return np.array([
            [s,s,s], [s,s,-s], [s,-s,s], [s,-s,-s],
            [-s,s,s], [-s,s,-s], [-s,-s,s], [-s,-s,-s],
            [0,0,0]
    ...
    def run(self):
        ...
        for i, idx in enumerate(tqdm(indices)):
            sample = dataset[idx]
            obj_id = int(sample['obj_id']) # 获取当前物体ID
            
            # 初始化该物体的统计槽
            if obj_id not in per_obj_stats:
                per_obj_stats[obj_id] = {'errors': [], 'total': 0}
            ...
            # [修改] 记录到总表 和 分表中
            per_obj_stats[obj_id]['errors'].append(error)
            ...
            if i % CONFIG["vis_interval"] == 0:
                ...
                real_box = self.get_real_box_pts(obj_id)
...
#         print("\n" + "="*60)
#         print(f"Model: {CONFIG['model_path']}")
#         print(f"{'Obj ID':<10} | {'Name (Approx)':<15} | {'Count':<5} | {'Acc(<10%)':<10} | {'Mean Err':<10}")
#         print("-" * 60)
        
#         # YCB 物体名称映射 (可选，方便看)
#         obj_names = {
#     1: "002_master_chef_can",    # 厨师长罐头
#     2: "003_cracker_box",       # 饼干盒
#     3: "004_sugar_box",         # 糖盒
#     4: "005_tomato_soup_can",   # 番茄汤罐头
#     5: "006_mustard_bottle",    # 芥末瓶
#     6: "007_tuna_fish_can",     # 金枪鱼罐头
#     7: "008_pudding_box",       # 布丁盒
#     8: "009_gelatin_box",       # 明胶/果冻盒
#     9: "010_potted_meat_can",   # 午餐肉罐头
#     10: "011_banana",           # 香蕉
#     11: "019_pitcher_base",     # 水壶/大水杯
#     12: "021_bleach_cleanser",  # 漂白剂/洗洁精
#     13: "024_bowl",             # 碗
#     14: "025_mug",              # 马克杯
#     15: "035_power_drill",      # 电钻
#     16: "036_wood_block",       # 木块
#     17: "037_scissors",         # 剪刀
#     18: "040_large_marker",     # 粗马克笔
#     19: "051_large_clamp",      # 大夹子
#     20: "052_extra_large_clamp",# 特大号夹子
#     21: "061_foam_brick"        # 泡沫砖
# }

#         # 遍历每个物体打印成绩
#         sorted_ids = sorted(per_obj_stats.keys())
#         for oid in sorted_ids:
#             stats = per_obj_stats[oid]
#             if stats['total'] == 0: continue
            
#             errs = np.array(stats['errors'])
#             # 计算该物体的准确率
#             # 注意：这里 diameter 统一用了 150mm，严谨的话应该用每个物体真实的直径
#             # 但为了横向对比，暂时统一也可以
#             acc = np.mean(errs < 0.1 * 150.0) * 100 
#             mean_err = np.mean(errs)
            
#             name = obj_names.get(oid, f"Obj {oid}")
#             print(f"{oid:<10} | {name:<15} | {stats['total']:<5} | {acc:6.2f}%    | {mean_err:.1f} mm")
            
#         print("-" * 60)
#         # 打印总平均 (Macro Average)
#         all_accs = [np.mean(np.array(per_obj_stats[oid]['errors']) < 15.0) * 100 for oid in sorted_ids]
#         print(f"MEAN      | All Classes     | {len(indices):<5} | {np.mean(all_accs):6.2f}%    | {np.mean(add_errors):.1f} mm")
        # print("="*60)

        # YCB 物体名称和直径映射（单位：毫米）
        obj_info = {
            1: {"name": "002_master_chef_can", "diameter": 102.10},
            2: {"name": "003_cracker_box", "diameter": 172.16},
            3: {"name": "004_sugar_box", "diameter": 102.10},
            4: {"name": "005_tomato_soup_can", "diameter": 102.10},
            5: {"name": "006_mustard_bottle", "diameter": 123.40},
            6: {"name": "007_tuna_fish_can", "diameter": 85.10},
            7: {"name": "008_pudding_box", "diameter": 72.50},
            8: {"name": "009_gelatin_box", "diameter": 72.50},
            9: {"name": "010_potted_meat_can", "diameter": 83.20},
            10: {"name": "011_banana", "diameter": 178.40},
            11: {"name": "019_pitcher_base", "diameter": 143.00},
            12: {"name": "021_bleach_cleanser", "diameter": 103.80},
            13: {"name": "024_bowl", "diameter": 165.18},
            14: {"name": "025_mug", "diameter": 118.50},
            15: {"name": "035_power_drill", "diameter": 164.81},
            16: {"name": "036_wood_block", "diameter": 82.60},
            17: {"name": "037_scissors", "diameter": 116.40},
            18: {"name": "040_large_marker", "diameter": 148.30},
            19: {"name": "051_large_clamp", "diameter": 111.70},
            20: {"name": "052_extra_large_clamp", "diameter": 180.00},
            21: {"name": "061_foam_brick", "diameter": 101.20}
        }
        
        # 遍历每个物体打印成绩
        sorted_ids = sorted(per_obj_stats.keys())
        print("=" * 100)
        print(f"{'ID':<4} | {'Object Name':<25} | {'Count':<6} | {'Mean Err':<8} | {'10%':<6} | {'15%':<6} | {'20%':<6} | {'50%':<6}")
        print("-" * 100)
        ...
        for oid in sorted_ids:
            ...
            # 获取物体直径
            if oid in obj_info:
                diameter = obj_info[oid]["diameter"]
                name = obj_info[oid]["name"]
            else:
                diameter = 100.0  # 默认值
                name = f"Obj_{oid}"
            
            # 计算不同阈值的准确率（相对于物体直径）
            acc_10 = np.mean(errs < 0.10 * diameter) * 100  # 10%直径内
            acc_15 = np.mean(errs < 0.15 * diameter) * 100  # 15%直径内
            acc_20 = np.mean(errs < 0.20 * diameter) * 100  # 20%直径内
            acc_50 = np.mean(errs < 0.50 * diameter) * 100  # 50%直径内
            
            mean_err = np.mean(errs)
            
            print(f"{oid:<4} | {name:<25} | {stats['total']:<6} | {mean_err:>7.1f}mm | "
                  f"{acc_10:>5.1f}% | {acc_15:>5.1f}% | {acc_20:>5.1f}% | {acc_50:>5.1f}%")
            
            # 收集统计信息
            total_samples += stats['total']
            class_acc_10.append(acc_10)
            class_acc_15.append(acc_15)
            class_acc_20.append(acc_20)
            class_acc_50.append(acc_50)
        ...
        # 打印总体统计（宏平均 - 类别平均）
        if class_acc_10:
            print(f"{'MEAN':<4} | {'All Classes (Macro)':<25} | {len(sorted_ids):<6} | "
                  f"{np.mean(mean_errors):>7.1f}mm | "
                  f"{np.mean(class_acc_10):>5.1f}% | {np.mean(class_acc_15):>5.1f}% | "
                  f"{np.mean(class_acc_20):>5.1f}% | {np.mean(class_acc_50):>5.1f}%")
            
            # 可选：打印微平均（按样本加权）
            all_errors = np.concatenate([per_obj_stats[oid]['errors'] for oid in sorted_ids])
            weighted_acc_10 = np.mean(all_errors < 0.10 * np.mean([obj_info[oid]["diameter"] for oid in sorted_ids if oid in obj_info])) * 100
            weighted_acc_15 = np.mean(all_errors < 0.15 * np.mean([obj_info[oid]["diameter"] for oid in sorted_ids if oid in obj_info])) * 100
            weighted_acc_20 = np.mean(all_errors < 0.20 * np.mean([obj_info[oid]["diameter"] for oid in sorted_ids if oid in obj_info])) * 100
            weighted_acc_50 = np.mean(all_errors < 0.50 * np.mean([obj_info[oid]["diameter"] for oid in sorted_ids if oid in obj_info])) * 100
            
            print(f"{'':<4} | {'All Classes (Micro)':<25} | {total_samples:<6} | "
                  f"{np.mean(all_errors):>7.1f}mm | "
                  f"{weighted_acc_10:>5.1f}% | {weighted_acc_15:>5.1f}% | "
                  f"{weighted_acc_20:>5.1f}% | {weighted_acc_50:>5.1f}%")
