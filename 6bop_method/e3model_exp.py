...
class AFDM(nn.Module):
    ...
    def forward(self, feat_2d, point_coords, point_feats):
        ...
        scatter_map = torch.zeros((B, C, H, W), device=device,dtype=point_feats.dtype)
...
class GMGPVNet(nn.Module):
    ...
    def forward(self, input_tensor, depth_tensor, template_tensor, event_points=None, 
                use_geo=True, use_gate=True, use_afdm=True):
        """
        ...
        # 1. 拆分输入
        # 即使不做几何分支，也要拆分，保证代码逻辑一致
        rgb_crop = input_tensor[:, :3, :, :] 
        mts_crop = input_tensor[:, 3:, :, :] 
        
        # 2. RGB 分支 (始终开启)
        # [关键] 必须同时获取 l1 (浅层) 和 l3 (深层)
        f_rgb_l1, f_rgb_l3 = self.backbone_rgb.forward_features(rgb_crop)
        
        # Template 处理 (始终开启)
        _, f_tpl_l3 = self.backbone_template.forward_features(template_tensor)
        ...
        if use_geo:
            # 正常计算几何特征
            geo_input = torch.cat([mts_crop, depth_tensor], dim=1)
            ...
            if use_gate:
                ...
                # Ablation: 简单相加，验证 Gated Fusion 的有效性
                f_2d = f_rgb_enhanced + f_geo_l3
        else:
            # Ablation: 不使用几何分支
            f_2d = f_rgb_enhanced
            
            # [关键] 制造全 0 的几何浅层特征，为了骗过后面的 Skip Layer
            # 必须保持维度一致，否则 skip_layer 报错
            f_geo_l1 = torch.zeros_like(f_rgb_l1) 

        # 4. AFDM 点云 (消融开关)
        if event_points is not None and use_afdm:
            p_coords = event_points[:, :, :2] 
            # 记得这里要归一化，保持和你之前训练代码一致
            points_norm = event_points.clone()
            points_norm[:, :, 0] /= 128.0
            points_norm[:, :, 1] /= 128.0
            p_feats = self.pointnet(points_norm)
            
            f_2d = self.afdm(f_2d, p_coords, p_feats)

        # 5. 上采样与跳跃连接 (必须保留！)
        f_up = self.upsample_block(f_2d) 
        
        # [关键] 拼接浅层特征
        # 如果 use_geo=False，f_geo_l1 是全0，这样不会影响结果，但保证了通道数匹配
        f_shallow_cat = torch.cat([f_rgb_l1, f_geo_l1], dim=1) 
        f_skip = self.skip_layer(f_shallow_cat)
        
        f_final = torch.cat([f_up, f_skip], dim=1) 
        f_final = self.head_conv(f_final)
        
        # 6. 输出
        return self.vector_head(f_final), self.seg_head(f_final)