...
CONFIG = {
    # --- 实验基本信息 ---
    "exp_name": "obj_20_rgb_only",   # 实验名建议改清楚点
    "target_obj_id": 20,
    
    # --- [核心] 消融实验开关 (Ablation Settings) ---
    # RGB Only:      use_geo=False, use_gate=False, use_afdm=False
    # RGB+Depth:     use_geo=True,  use_gate=False, use_afdm=False
    # RGB+D+Gate:    use_geo=True,  use_gate=True,  use_afdm=False
    # Full (Points): use_geo=True,  use_gate=True,  use_afdm=True
    "ablation": {
        "use_geo":  False,  # 关闭几何分支 (Depth/MTS)
        "use_gate": False,  # 关闭门控
        "use_afdm": False   # 关闭点云扩散
    },
    "use_event_points": False, # Dataset层面是否读取点云 (配合 use_afdm)
    # -----------------------------------
    ...
    "base_save_dir": "./finalexperiment/exp/20",
    "visualize_freq": 1 
}
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        for batch in pbar:
            ...
            with autocast(device_type='cuda'):
                ...
                pred_vec, pred_mask = model(
                    inputs, depth, template, 
                    event_points=event_points,
                    use_geo=CONFIG["ablation"]["use_geo"],
                    use_gate=CONFIG["ablation"]["use_gate"],
                    use_afdm=CONFIG["ablation"]["use_afdm"]
                )
