...
class HybridPoseNet(nn.Module):
    def __init__(self, feature_dim=512, nhead=8, num_layers=2):
        ...
        # 1. CNN Backbone: 使用 ResNet18
        # 我们需要去掉最后的全连接层(fc)和平均池化层(avgpool)
        resnet = models.resnet18(pretrained=True)
        self.backbone = nn.Sequential(*list(resnet.children())[:-2])
        
        # ResNet18最后输出的特征图通道数是 512
        self.feature_dim = feature_dim
        
        # 2. Transformer Encoder
        # d_model: 输入特征维度 (512)
        # nhead: 注意力头数 (8)
        # dim_feedforward: 内部隐藏层大小 (2048)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=feature_dim, 
            nhead=nhead, 
            dim_feedforward=2048, 
            batch_first=True  # 关键：让输入格式变成 [Batch, Seq, Feature]
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        
        # 3. 回归头 (Regression Head)
        # 输入是 Transformer 输出的平均值 (512维)
        self.regress_head = nn.Sequential(
            nn.Linear(feature_dim, 256),
            nn.ReLU(),
            nn.Dropout(0.5), # 防止过拟合
            nn.Linear(256, 7) # 3个位置 + 4个四元数
        )
    ...
    def forward(self, x):
        ...
        # features shape: [Batch, 512, 7, 7] (对于224x224输入)
        
        b, c, h, w = features.shape
        
        # --- 步骤2: 形状变换 (Flatten) ---
        # 我们要把 spatial dimensions (7x7) 拉平成序列 (49)
        # 变换目标: [Batch, 49, 512]
        features = features.view(b, c, h * w).permute(0, 2, 1)
        
        # --- 步骤3: Transformer 全局建模 ---
        # Transformer 就在这里发挥作用，它会计算这 49 个区域之间的关联
        trans_feat = self.transformer(features)
        # trans_feat shape: [Batch, 49, 512]
        
        # --- 步骤4: 聚合 (Global Average Pooling) ---
        # 把 49 个区域的信息浓缩成 1 个向量
        global_feat = torch.mean(trans_feat, dim=1)
        # global_feat shape: [Batch, 512]
        
        # --- 步骤5: 回归预测 ---
        out = self.regress_head(global_feat)
        return out
...
def main():
    ...
    print("正在初始化 Hybrid CNN-Transformer 模型...")
    model = HybridPoseNet(num_layers=2).to(DEVICE) # 2层 Transformer 足够了，跑得快
    ...
    torch.save(model.state_dict(), "hybrid_pose_transformer.pth")
